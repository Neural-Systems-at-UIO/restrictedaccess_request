//if the code does not find datasets in the IN_PROGRESS space, try to finish this code and replace 
//the file contactDataKG.js
...
export async function contactInfoKG(queryID, datasetID, headers) {
    try{
        ...
        const dataKG = await fetchKGjson(queryID, datasetID, headers, QUERY_PARAMS);
        try{
            const custodianDatasetVersion = dataKG[0]['data'][0]['custodian'];
            if (dataKG[0]['data'][0]) {
                console.log(dataKG[0]['data'][0]);
                return custodianDatasetVersion;
              } else {
                throw new Error('Could not find data in IN_PROGRESS');
              };
        } catch(error) {
            throw new Error(`Data are not in IN_PROGRESS: ${error.message}`);
        };
